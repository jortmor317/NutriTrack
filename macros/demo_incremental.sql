{% macro demo_incremental() %}
    ...
    {% set insert_subscription %}
        ...
        SELECT
            MD5(CURRENT_TIMESTAMP()::VARCHAR || RANDOM()::VARCHAR),
            ...
            CURRENT_TIMESTAMP();
    {% endset %}
    {% do run_query(insert_subscription) %}
    ...
    {% set insert_food_log %}
        ...
        SELECT
            MD5(CURRENT_TIMESTAMP()::VARCHAR || RANDOM()::VARCHAR || '2'),
            ...
            100,
            'g',
            (SELECT FDC_ID FROM {{ source('nutritrack', 'FOOD_LOG_ITEMS') }}
             WHERE FDC_ID IS NOT NULL LIMIT 1);
    {% endset %}
